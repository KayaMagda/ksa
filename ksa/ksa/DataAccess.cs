...
namespace ksa
{
    ...
    public class DataAccess
    {
        ...
        public static void CreateDatabase()
        {
            if (!File.Exists(dbFileName))
            {
                ...
                string query = @"CREATE TABLE Kunde (
                                ...
                                CREATE TABLE Tonnennummer (
                                nr bigint PRIMARY KEY
                                                         );

                               INSERT INTO Tonnennummer (nr) VALUES (6456785);
        ...
        public static long GetTonnennummer()
        {
            var query = "SELECT nr FROM Tonnennummer";
        ...
        public static void UpdateTonnennummer(long tonnennummer)
        {
            var query = "UPDATE Tonnennummer SET nr = @nr";

            using SQLiteConnection connection = GetOpenConnection();
            using SQLiteCommand command = new SQLiteCommand(query, connection);

            command.Parameters.AddWithValue("@nr", tonnennummer);
